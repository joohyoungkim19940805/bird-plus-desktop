	...
	constructor(pageName){
		...
		*/
		const indexedDBHandler = new IndexedDBHandler({
			dbName: 'fileDB-'+pageName,
			storeName: 's3Memory-'+pageName,
			columnInfo: {
		...
		const imageOrVideoCallback = async (targetTools) => {
			...
			startPromise.then((filePreview) => {
				console.log(targetTools);
				let getSignData = `${targetTools.dataset.room_id}:${targetTools.dataset.workspace_id}:${targetTools.dataset.new_file_name}:${targetTools.closest('[data-account_name]')?.dataset.account_name}`
						...
						.then(async blob => {
							...
							return dbOpenPromise.then( async () => {
								return indexedDBHandler.addItem({
									...
									roomId: targetTools.dataset.room_id,
									workspaceId: targetTools.dataset.workspace_id
								}).then(()=>{
